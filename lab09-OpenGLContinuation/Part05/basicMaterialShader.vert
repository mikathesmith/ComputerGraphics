...
//In addition to the texture coordinates and the vertex position in clip space, the normal of the vertex, the eye direction and the light direction in camera space will be passed to the fragment shader. For computation, we use the model matrix M, the view matrix V and the model-view-projection matrix MVP to transform into camera space and clip space. 
