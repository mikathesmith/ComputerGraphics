 ...
 *
 * Simple Demonstration for getting started with opening a simple window that displays a red triangle
 *
 */
...
int main( void )
{
    ...
    //each object has a shader object as a member variable. later used for transforming the triangles vertices to the screen using the model-view projection matrix (MVP). 
    ...
    while( glfwGetKey(window, GLFW_KEY_ESCAPE ) != GLFW_PRESS && glfwWindowShouldClose(window) == 0 ){// Clear the screen
		...
		myScene->render(myCamera); // all objects will be rendered using this method
